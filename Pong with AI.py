...
def moveUp2(*args):
    canv.move(paddle2, 0, -(random.randint(1,5)))        
def moveDown1(*args):
    canv.move(paddle1, 0, 15)        
def moveDown2(*args):
    canv.move(paddle2, 0, random.randint(1,5))    
...
def startgame(*args):
    ...
    if gameover==False:
        ...
        Player2scoreLabel.configure(text="Computer score: "+ str(Player2score))
        gameover = False
        moveBall()
...
#        end game if player 1 or 2 wins
        ...
        if Player1score==3:
            ...
            Scorer = "The Computer is the winner! Press start to play again or quit to kill yourself!"
            ScoreLabel.configure(text=Scorer)
        gameover = True
